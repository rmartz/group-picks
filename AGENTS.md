...
- **Pin every third-party GitHub Action to a full 40-char commit SHA with a trailing version comment** (e.g. `actions/checkout@9c091bb… # v7.0.0`, not `@v7`). A tag can be repointed at attacker code (supply-chain attack); a SHA is immutable. The comment is **required**, not decorative: Dependabot reads the version from it to keep the SHA updated, so a bare SHA with no comment would freeze the action forever. It must be a **full `major.minor.patch`** (`# v7.0.0`) — partials like `# v6` or `# v7.0` are rejected, since Dependabot's handling of non-full-semver comments is inconsistent. The CI check enforces **both** the SHA and the full-semver comment: it is the `action-pins` check from `@rmartz/repo-hygiene`, run on every PR by `.github/workflows/repo-hygiene.yml` and held at `severity: error` in `.repo-hygiene.yml`. It is the sole enforcement point — there is no local pre-check to run, and the retired home-grown pair (`.github/workflows/action-pins.yml` and `pnpm pins:actions` → `scripts/check-action-pins.mjs`, both removed when the hygiene suite was adopted in #485) should not be reintroduced. The full-semver `package.json` pin check is the npm-side parallel and is still home-grown — `package-pins.yml` → `pnpm pins:check`. Local `./…` composite actions need no pin; a `docker://` image must be pinned by `@sha256:` digest.
...
- Storybook CI is two thin callers of the shared [`rmartz/storybook-ci`](https://github.com/rmartz/storybook-ci) reusable workflows, pinned by SHA: `storybook-tests.yml` (gating — the required `storybook-tests / Storybook Tests` check) and `storybook-screenshots.yml` (advisory gallery). The shared `storybook-tests / Storybook Build` job also runs the built-bundle render canary (`scripts/check-storybook-render.mjs`, #381) via `build-command` + `build-needs-browser: true`; there is no in-repo Storybook build job, and the local `setup` action installs no browsers. Background, inputs, and the pin-bump path: [`docs/storybook-ci.md`](docs/storybook-ci.md).
- When a PR changes a story **or a component co-located with one**, the `Capture Storybook Screenshots` job posts a PNG per resolved story in a single update-in-place PR comment — a **visual acceptance aid** to eyeball an intended UI change (it is **advisory**, never a merge gate; the always-on `Storybook Tests` suite is what catches regressions). Images are GitHub user-attachments, so there is no image branch and nothing to clean up.
- The gallery needs the `STORYBOOK_SCREENSHOT_PAT` Actions secret (a fine-grained PAT scoped to this repo, `Pull requests: Read and write`). Without it the job posts a non-blocking advisory comment and skips the capture rather than failing. See [`docs/storybook-ci.md`](docs/storybook-ci.md#the-screenshot-pat).
- **Before/After is off** (it existed under the old in-repo capture, #403): the shared workflow renders the PR only by default. Upstream now supports it as the opt-in `capture-base: true` input ([rmartz/storybook-ci#22](https://github.com/rmartz/storybook-ci/issues/22)); this repo has not enabled it yet.
