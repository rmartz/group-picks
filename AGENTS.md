...
- Storybook CI is two thin callers of the shared [`rmartz/storybook-ci`](https://github.com/rmartz/storybook-ci) reusable workflows, pinned by SHA: `storybook-tests.yml` (gating — the required `storybook-tests / Storybook Tests` check) and `storybook-screenshots.yml` (advisory gallery). `Storybook Build` stays in `ci-actions.yml` because it also runs the built-bundle render canary (#381). Background, inputs, and the pin-bump path: [`docs/storybook-ci.md`](docs/storybook-ci.md).
...
- **Before/After is gone** (it existed under the old in-repo capture, #403): the shared workflow renders the PR only. Tracked upstream in [rmartz/storybook-ci#22](https://github.com/rmartz/storybook-ci/issues/22); when it lands, adopting it here is a pin bump.
